...
func renvoScan(src []byte, toks *renvoTokens) {
	...
	for i < srcLen {
		...
		// The ARM performance compiler parses large, conventionally formatted
		// sources. Consume indentation and newlines as a run to keep that fixed
		// compiler below its runtime cap without growing every target.
...
func renvoConvertConstScalar(renvoNativeIntSize int, value int, sourceKind int, destKind int) int {
	...
	return renvoConvertConstInt(renvoNativeIntSize, value, destKind)
}
			...
			if conversionType != 0 {
				...
				if renvoTypeKindIsScalarValue(resolved.kind) {
					...
					if result.ok {
						...
						renvoNonNil(source)
						result.value = renvoConvertConstScalar(g.c.renvoNativeIntSize, result.value, source.kind, resolved.kind)
					}
					*out = result
					return
				}
...
func renvoFinalizeTypeLayouts(m *renvoMeta) {
	...
	for i := 0; i < len(m.types); i++ {
		...
		if t.kind == renvoTypeStruct {
			for j := 0; j < t.count; j++ {
				if renvoResolveType(m, m.fields[t.first+j].typ).resolved == renvoStructLayoutMarker {
		...
		current := m.types[i]
		if current.kind != renvoTypeStruct || current.nativeAlign > 0 {
			continue
		}
		offset := 0
		for j := 0; j < current.count; j++ {
			fieldIndex := current.first + j
...
func renvoNativeTypeLayout(m *renvoMeta, typ int) int {
	...
	if t.kind == renvoTypeNamed {
		...
		for i := 0; i < t.count; i++ {
			...
			fieldResolved := renvoResolveType(m, fieldType)
			fieldSize := 0
			fieldAlign := 0
			if t.resolved == renvoStructLayoutHost && fieldResolved.kind == renvoTypeStruct &&
				fieldResolved.resolved != renvoStructLayoutMarker && fieldResolved.resolved != renvoStructLayoutHost {
				fieldSize = renvoTypeSize(m, fieldType)
				fieldAlign = renvoBackendValueSlotSize
			} else {
...
func renvoAsmLoadPrimaryIndexTertiaryScalarOrPointer(a *renvoAsm, kind int) {
	renvoNonNil(a)
	elemSize := renvoScalarKindSize(a.c.renvoNativeIntSize, kind)
...
func renvoEmitLinearAssign(g *renvoLinearGen, stmt *renvoStmt) bool {
	...
	if globalOffset >= 0 && (renvoTypeIsStruct(meta, targetType) || targetResolved.kind == renvoTypeInterface || g.c.renvoNativeIntSize == 4 && renvoTypeKindIsWideInt(targetResolved.kind)) {
...
func renvoEmitTempToTarget(g *renvoLinearGen, kind int, targetStart int, targetEnd int, tempOffset int, tempType int) bool {
	...
	if root.kind == renvoExprIndex {
		...
		if baseType.kind == renvoTypeMap {
			...
			if g.c.renvoNativeIntSize == 4 && renvoTypeKindIsWideInt(targetType.kind) {
				...
				renvoAsmStorePrimaryMemSecondaryDispSize(&g.asm, 16, renvoScalarKindSize(g.c.renvoNativeIntSize, targetType.kind))
			}
			return true
		}
		if !renvoEmitIndexAddressPrimary(g, ep, rootIndex) {
...
func renvoEmitSliceLiteralBacking(g *renvoLinearGen, ep *renvoExprParse, idx int, sliceType int, backingOff int) bool {
	...
	for i := 0; i < e.argCount; i++ {
		...
		if elemResolved.kind == renvoTypeArray || elemResolved.kind == renvoTypeMap || elemResolved.kind == renvoTypeSlice || g.c.renvoNativeIntSize == 4 && renvoTypeKindIsWideInt(elemResolved.kind) {
			tempOffset := renvoAddUnnamedLocal(g, elemType)
...
func renvoEmitCompositeFieldToStack(g *renvoLinearGen, ep *renvoExprParse, idx int, fieldType int, destOffset int) bool {
	...
	if fieldResolved.kind == renvoTypeStruct || fieldResolved.kind == renvoTypeInterface || g.c.renvoNativeIntSize == 4 && renvoTypeKindIsWideInt(fieldResolved.kind) {
...
func renvoEmitTypedLocalArgReverse(g *renvoLinearGen, offset int, typ int) int {
	...
	renvoNonNil(t)
	if renvoStructArgByReference(g, t.kind) {
		renvoAsmAddressPrimaryStack(&g.asm, offset)
		renvoAsmPushPrimary(&g.asm)
		return 1
	}
...
func renvoStructArgByReference(g *renvoLinearGen, kind int) bool {
	return renvoPreparedBackend != 0 && g.c.renvoNativeIntSize == 4 && g.c.renvoTargetArch != renvoArchWasm32 && kind == renvoTypeStruct
}
...
func renvoEmitMethodReceiverArgReverse(g *renvoLinearGen, ep *renvoExprParse, idx int, receiverType int) int {
	...
	if renvoStructArgByReference(g, receiver.kind) {
...
func renvoEmitStructArgReverse(g *renvoLinearGen, ep *renvoExprParse, idx int, typ int) int {
	...
	e := &ep.exprs[idx]
	if renvoStructArgByReference(g, renvoResolveType(meta, typ).kind) {
		if !renvoEmitAddressPrimary(g, ep, idx) {
			offset := renvoAddUnnamedLocal(g, typ)
			if !renvoEmitTypedAssign(g, ep, idx, offset) {
				return -1
...
func renvoEnsureAppendScalarHelper(g *renvoLinearGen, elemKind int) int {
	renvoNonNil(g)
	if renvoScalarKindSize(g.c.renvoNativeIntSize, elemKind) == 1 {
...
func renvoEmitIndexExpr(g *renvoLinearGen, ep *renvoExprParse, idx int) bool {
	...
	if baseResolved.kind == renvoTypeMap {
		...
		renvoNonNil(valueType)
		renvoAsmLoadPrimaryMemSecondaryDispSize(a, 16, renvoScalarKindSize(g.c.renvoNativeIntSize, valueType.kind))
...
func renvoAsmScratchBssAddr(a *renvoAsm, bssOff int) {
	renvoNonNil(a)
	if renvoPreparedBackend != 0 {
		renvoRTGDirectAddress(a, renvoRTGScratch, renvoRTGAsmBSSAddress(bssOff))
		return
	}
	if a.c.renvoTargetArch == renvoArchWasm32 {
...
func renvoAsmCopyPrimaryToCallWord4(a *renvoAsm) {
	renvoNonNil(a)
	if renvoPreparedBackend != 0 {
		renvoRTGDirectMove(a, renvoRTGCallWord4, renvoRTGPrimary)
		return
	}
	if a.c.renvoTargetArch == renvoArchWasm32 {
...
func renvoAsmCopyPrimaryToCallWord5(a *renvoAsm) {
	renvoNonNil(a)
	if renvoPreparedBackend != 0 {
		renvoRTGDirectMove(a, renvoRTGCallWord5, renvoRTGPrimary)
		return
	}
	if a.c.renvoTargetArch == renvoArchWasm32 {
...
func renvoAsmLoadQwordPrimaryIndexTertiary8(a *renvoAsm) {
	...
	if renvoPreparedBackend != 0 {
		renvoRTGDirectLoadNative(a, renvoRTGPrimary,
			renvoRTGAsmAddress(renvoRTGPrimary, renvoRTGTertiary, 0, 8))
		return
	}
	if a.c.renvoTargetArch == renvoArchWasm32 {
...
func renvoEmitSwitchStringCaseTest(g *renvoLinearGen, valueOffset int, lenOffset int, ep *renvoExprParse, idx int, matchLabel int) bool {
	renvoNonNil(g, ep)
	if g.c.renvoTargetArch == renvoArch386 {
		return renvoEmitWideSwitchStringCaseTest(g, valueOffset, lenOffset, ep, idx, matchLabel)
	}
...
func renvoEmitCompareJump(g *renvoLinearGen, ep *renvoExprParse, idx int, label int, jumpIfTrue bool) bool {
	...
	if g.c.renvoTargetArch == renvoArch386 {
		return renvoEmitWideCompareJump(g, ep, e, label, jumpIfTrue)
	}
...
func renvoEmitWideStructReturnExpr(g *renvoLinearGen, ep *renvoExprParse, idx int) bool {
	...
	if e.kind == renvoExprIdent {
		...
		if renvoPreparedBackend != 0 {
			...
			renvoAsmAddressPrimaryStack(a, g.locals[localIndex].offset)
			renvoAsmStorePrimaryStack(a, source)
			renvoAsmLoadPrimaryStack(a, g.returnStruct)
			renvoAsmStorePrimaryStack(a, dest)
			renvoEmitRTGCopyStructAddressToAddress(g, resultType, source, dest)
			return true
		}
	...
	if e.kind == renvoExprIndex {
		...
		if renvoPreparedBackend != 0 {
			source := renvoAddUnnamedLocal(g, renvoTypeInt)
			dest := renvoAddUnnamedLocal(g, renvoTypeInt)
			renvoAsmStoreSecondaryStack(a, source)
			renvoAsmLoadPrimaryStack(a, g.returnStruct)
			renvoAsmStorePrimaryStack(a, dest)
			renvoEmitRTGCopyStructAddressToAddress(g, resultType, source, dest)
			return true
		...
		for at := 0; at < size; at += 8 {
			...
			if at == 0 {
				...
				if renvoPreparedBackend != 0 {
					renvoRTGDirectStoreNative(a,
						renvoRTGAsmAddress(renvoRTGTertiary, RTGNoRegister, at, 1),
						renvoRTGPrimary)
				} else {
...
func renvoEmitWideStringPtrExpr(g *renvoLinearGen, ep *renvoExprParse, idx int) bool {
	p := g.prog
	meta := g.meta
	a := &g.asm
	e := &ep.exprs[idx]
...
func renvoEmitWideScalarToLocal(g *renvoLinearGen, ep *renvoExprParse, idx int, offset int, sourceKind int) bool {
	...
	renvoAsmStorePrimaryStack(&g.asm, offset)
	if sourceKind == renvoTypeInt || sourceKind == renvoTypeInt8 || sourceKind == renvoTypeInt16 || sourceKind == renvoTypeInt32 || sourceKind == renvoTypeInt64 {
...
func renvoEmitWideExprToLocal(g *renvoLinearGen, ep *renvoExprParse, idx int, offset int, destKind int) bool {
	...
	if e.kind == renvoExprCall {
		...
		if conversionType != 0 {
			...
			if g.c.renvoNativeIntSize == 4 && renvoTypeKindIsWideInt(source.kind) || argKind == renvoExprInt || argKind == renvoExprUnary || argKind == renvoExprBinary {
...
func renvoEmitAmd64FloatBinaryExpr(g *renvoLinearGen, ep *renvoExprParse, idx int) bool {
	renvoNonNil(g, ep)
	p := g.prog
	a := &g.asm
	e := &ep.exprs[idx]
...
func renvoEmitAarch64FloatBinaryExpr(g *renvoLinearGen, ep *renvoExprParse, idx int) bool {
	p := g.prog
	a := &g.asm
	e := &ep.exprs[idx]
	if renvoTokCharIs(p, e.tok, '*') {
...
func renvoEmitArmFloatBinaryExpr(g *renvoLinearGen, ep *renvoExprParse, idx int) bool {
	p := g.prog
	a := &g.asm
	e := &ep.exprs[idx]
	if renvoTokCharIs(p, e.tok, '*') {
...
func renvoEmitFloatBinaryExpr(g *renvoLinearGen, ep *renvoExprParse, idx int) bool {
	...
	if g.c.renvoTargetArch == renvoArchAarch64 {
		return renvoEmitAarch64FloatBinaryExpr(g, ep, idx)
	}
	if g.c.renvoTargetArch == renvoArchArm {
		return renvoEmitArmFloatBinaryExpr(g, ep, idx)
	}
	if g.c.renvoTargetArch == renvoArch386 {
...
func renvoEmitNativeCompareJump(g *renvoLinearGen, ep *renvoExprParse, e *renvoExpr, label int, jumpIfTrue bool) bool {
	...
	if usesFloat && !renvoEmitScalarExprForKind(g, ep, rightIndex, renvoTypeFloat64) || !usesFloat && !renvoEmitIntExpr(g, ep, rightIndex) {
		return false
	}
	renvoAsmPushPrimary(&g.asm)
	if usesFloat && !renvoEmitScalarExprForKind(g, ep, leftIndex, renvoTypeFloat64) || !usesFloat && !renvoEmitIntExpr(g, ep, leftIndex) {
		return false
	}
	renvoAsmPopTertiary(&g.asm)
	if renvoPreparedBackend != 0 {
