...
func renvoAsmAddExternalImportName(a *renvoAsm, name string) int {
	...
	for i := 0; i+1 < len(a.kernelImportOffsets); i += 2 {
		...
		for j := 0; j < len(name); j++ {
			if a.kernelImportNames[start+j] != name[j] {
				match = false
			}
		}
		if match {
...
func renvoParseTypeInto(m *renvoMeta, p *renvoProgram, start int, end int, result *renvoTypeResult) {
	...
	if renvoTokIsKind(p, start, renvoTokStruct) && renvoTokCharIs(p, start+1, '{') {
		...
		firstField := len(m.fields)
		fieldEnd := firstField + count
		if fieldEnd > cap(m.fields) {
			m.fields = append(m.fields, make([]renvoFieldInfo, fieldEnd-len(m.fields))...)
		} else {
			renvoTruncFields(&m.fields, fieldEnd)
		}
		fieldIndex := 0
		offset := 0
...
func renvoBuiltinTypeFromToken(p *renvoProgram, tokIndex int) int {
	...
	if renvoBytesEqualText(p.src, int(tok.start), int(tok.end), "float32") || renvoBytesEqualText(p.src, int(tok.start), int(tok.end), "float64") {
		return renvoTypeFloat64
	}
...
func renvoTypeKindUsesMemory(kind int) bool {
	return kind == renvoTypeString || (kind >= renvoTypeSlice && kind <= renvoTypeStruct) || kind == renvoTypeArray || kind == renvoTypeInterface || kind == renvoTypeComplex
}
...
func renvoTypeUsesHiddenResult(m *renvoMeta, typ int) bool {
	...
	t := renvoResolveType(m, typ)
	renvoNonNil(t)
	return t.kind == renvoTypeStruct || t.kind == renvoTypeArray || t.kind == renvoTypeInterface || m.c.renvoNativeIntSize == 4 && renvoTypeKindIsWideInt(t.kind)
}
...
func renvoBindFunctionParams(g *renvoLinearGen, fnIndex int) {
	...
	for at := 0; at < fn.paramCount; at++ {
		...
		if paramType.kind == renvoTypeComplex {
			renvoStoreIncomingCallWord(g, callWord, offset)
			renvoStoreIncomingCallWord(g, callWord+1, offset-renvoBackendValueSlotSize)
			callWord += 2
			continue
...
func renvoEmitBareReturnValues(g *renvoLinearGen) bool {
	...
	if fn.resultCount == 1 {
		...
		if resolved.kind == renvoTypeComplex {
			renvoAsmLoadPrimarySecondaryStack(&g.asm, offset, offset-renvoBackendValueSlotSize)
			return true
		}
...
func renvoAsmPushPrimary(a *renvoAsm) {
	...
	fold := a.c.renvoTargetArch == renvoArchAmd64 && load > 0 && load/8 == len(a.code)
...
func renvoAddStringData(g *renvoLinearGen, msg []byte) int {
	return renvoAddStringDataAligned(g, msg, 1)
}
...
func renvoEmitLinearStmtCore(g *renvoLinearGen, stmt *renvoStmt) bool {
	...
	if stmt.kind == renvoStmtReturn {
		...
		if renvoTypeUsesHiddenResult(g.meta, resultType) {
			...
			if resultResolved.kind == renvoTypeComplex {
...
func renvoEmitLinearIf(g *renvoLinearGen, stmt *renvoStmt) bool {
	...
	thenExits := thenReturns || renvoRangeEndsControlExit(p, stmt.bodyStart, stmt.bodyEnd)
	var flowThen []int
	if flowCount > 0 {
	...
	elseExits := elseReturns || renvoRangeEndsControlExit(p, stmt.elseStart, stmt.elseEnd)
	if flowCount > 0 {
...
func renvoEmitLinearSimpleRange(g *renvoLinearGen, start int, end int) bool {
	...
	ep := renvoNewExprParse()
	rootIndex := renvoParseExpressionRoot(ep, p, start, end)
	return rootIndex >= 0 && renvoEmitIntExpr(g, ep, rootIndex)
}
func renvoEmitLinearIncDec(g *renvoLinearGen, start int, end int) bool {
	...
	if root.kind == renvoExprIdent {
		...
		if localOffset >= 0 {
			...
			if renvoPreparedBackend != 0 || g.c.renvoTargetArch == renvoArchAarch64 || g.c.renvoTargetArch == renvoArchArm || g.c.renvoTargetArch == renvoArchWasm32 {
...
func renvoEmitLinearCompoundLValue(g *renvoLinearGen, stmt *renvoStmt, assignTok int) int {
	...
	if lhsRoot.kind == renvoExprIndex {
		...
		renvoAsmStorePrimaryStack(a, addrOffset)
		if g.c.renvoNativeIntSize == 4 && renvoTypeKindIsWideInt(elemType.kind) {
	...
	if lhsRoot.kind == renvoExprSelector {
		...
		if g.c.renvoNativeIntSize == 4 && renvoTypeKindIsWideInt(lhsResolved.kind) {
...
func renvoEmitLinearAssign(g *renvoLinearGen, stmt *renvoStmt) bool {
	...
	if assignTok > stmt.startTok && renvoTokCharIs(p, assignTok, '=') && (startKind != renvoTokIdent || assignTok != stmt.startTok+1) {
		...
		if renvoParseExpressionOK(lhs, p, stmt.startTok, assignTok) {
			...
			if lhsRoot.kind == renvoExprIndex {
				...
				if renvoTypeKindUsesMemory(elemType.kind) || g.c.renvoNativeIntSize == 4 && renvoTypeKindIsWideInt(elemType.kind) {
			...
			if lhsRoot.kind == renvoExprSelector && (renvoTypeKindUsesMemory(lhsResolved.kind) || g.c.renvoNativeIntSize == 4 && renvoTypeKindIsWideInt(lhsResolved.kind)) {
				rhs := renvoNewExprParse()
				renvoNonNil(rhs)
				rhsIndex := renvoParseExpressionRoot(rhs, p, assignTok+1, stmt.endTok)
				if rhsIndex < 0 {
	...
	trackLocalConst := globalOffset < 0 && fieldStackOffset < 0 &&
		(declaresLocal || renvoTopLevelAssignmentDominates(g, stmt.startTok)) &&
	...
	if compoundAssign {
		...
		if g.c.renvoNativeIntSize == 4 && renvoTypeKindIsWideInt(targetResolved.kind) {
			valueOffset := offset
			if globalOffset >= 0 {
				valueOffset = renvoAddUnnamedLocal(g, targetType)
		...
		if g.c.renvoTargetArch == renvoArchAmd64 && memoryOp != 0 && globalOffset < 0 && fieldStackOffset < 0 && renvoTypeIsInt(meta, targetType) {
	...
	if globalOffset >= 0 && (renvoTypeIsStruct(meta, targetType) || targetResolved.kind == renvoTypeInterface || targetResolved.kind == renvoTypeComplex || g.c.renvoNativeIntSize == 4 && renvoTypeKindIsWideInt(targetResolved.kind)) {
...
func renvoEmitMultiAssign(g *renvoLinearGen, stmt *renvoStmt, assignTok int) bool {
	...
	if stmt.kind == renvoStmtAssign && lhsCount > 1 {
		for i := 0; i < lhsCount; i++ {
			...
			root := &ep.exprs[rootIndex]
			if root.kind != renvoExprIdent && renvoEmitAddressPrimary(g, ep, rootIndex) {
...
func renvoEmitTempToMapEntry(g *renvoLinearGen, tempOffset int, elemType int) {
	...
	if g.c.renvoNativeIntSize == 4 && renvoTypeKindIsWideInt(elem.kind) {
...
func renvoEmitTypedAssign(g *renvoLinearGen, ep *renvoExprParse, idx int, offset int) bool {
	...
	if g.c.renvoNativeIntSize == 4 && renvoTypeKindIsWideInt(destResolved.kind) {
...
func renvoEmitSliceLiteralBacking(g *renvoLinearGen, ep *renvoExprParse, idx int, sliceType int, backingOff int) bool {
	...
	for i := 0; i < e.argCount; i++ {
		...
		if elemResolved.kind == renvoTypeArray || elemResolved.kind == renvoTypeSlice || g.c.renvoNativeIntSize == 4 && renvoTypeKindIsWideInt(elemResolved.kind) {
			tempOffset := renvoAddUnnamedLocal(g, elemType)
...
func renvoEmitCompositeFieldToStack(g *renvoLinearGen, ep *renvoExprParse, idx int, fieldType int, destOffset int) bool {
	...
	if fieldResolved.kind == renvoTypeStruct || fieldResolved.kind == renvoTypeInterface || fieldResolved.kind == renvoTypeComplex || g.c.renvoNativeIntSize == 4 && renvoTypeKindIsWideInt(fieldResolved.kind) {
		tempOffset := renvoAddUnnamedLocal(g, fieldType)
		if !renvoEmitTypedAssign(g, ep, idx, tempOffset) {
			return false
		}
...
func renvoEmitUserCall(g *renvoLinearGen, ep *renvoExprParse, idx int) bool {
	...
	if fn.nameEnd > fn.nameStart+15 &&
		...
		if getUserWidth != 0 && e.argCount == 2 && g.c.objectFile && g.c.renvoTargetArch == renvoArchAmd64 {
			...
			renvoAsmAddAbsReloc(&g.asm, relocationAt, externalID, renvoKernelAmd64RelocationImport)
...
func renvoEmitCObjectFunctionPointerCall(g *renvoLinearGen, functionType *renvoTypeInfo, handleOffset int, argOffsets []int, resultOffset int) bool {
	...
	for i := 0; i < len(argOffsets); i++ {
		...
		if param.kind == renvoTypeStruct {
			if renvoPreparedBackend != 0 || !renvoObjectCABIIntegerAggregate(g.meta, paramType) {
				return false
			}
			hasAggregate = true
...
func renvoCallWordSize(g *renvoLinearGen, typ int) int {
	t := renvoResolveType(g.meta, typ)
	if t.kind == renvoTypeArray || g.c.renvoNativeIntSize == 4 && (renvoTypeKindIsWideInt(t.kind) || t.kind == renvoTypeStruct && renvoTypeNeedsDenseCallWords(g.meta, typ)) {
...
func renvoTypeNeedsDenseCallWords(meta *renvoMeta, typ int) bool {
	...
	t := renvoResolveType(meta, typ)
	return renvoTypeKindIsWideInt(t.kind) || t.kind == renvoTypeArray || t.kind == renvoTypeStruct && t.resolved == renvoStructLayoutDense
}
...
func renvoEmitCallParamArgReverse(g *renvoLinearGen, ep *renvoExprParse, idx int, paramIndex int) int {
	...
	if paramIndex >= 0 && paramIndex < len(meta.params) {
		...
		if resolved.kind == renvoTypeStruct || resolved.kind == renvoTypeArray || g.c.renvoNativeIntSize == 4 && renvoTypeKindIsWideInt(resolved.kind) {
...
func renvoEmitIndexAddressPrimary(g *renvoLinearGen, ep *renvoExprParse, indexIdx int) bool {
	...
	pointerElements := false
	if pointerArray {
		pointerType := sliceType
		sliceType = renvoResolveType(meta, pointerType.elem)
		if sliceType.kind != renvoTypeArray && sliceType.kind != renvoTypeSlice {
			pointerArray = false
			pointerElements = true
			sliceType = pointerType
		}
	}
	if !pointerElements && sliceType.kind != renvoTypeArray && sliceType.kind != renvoTypeSlice {
		return false
	}
...
func renvoEmitUnsignedPrimaryTertiaryOp(g *renvoLinearGen, tok int, kind int) bool {
	...
	renvoEmitRuntimeNonNilPrimary(g)
	if g.c.renvoNativeIntSize == 4 && (renvoPreparedBackend != 0 || g.c.renvoTargetArch == renvoArchWasm32) {
...
func renvoEmitWideCompareOperand(g *renvoLinearGen, ep *renvoExprParse, idx int, useFloat bool) bool {
	if useFloat {
...
func renvoObjectStoreConstant(g *renvoLinearGen, ep *renvoExprParse, idx int, typ int, data []byte, offset int, objectOffset int) bool {
	...
	if resolved.kind == renvoTypeFloat64 {
		scaled, ok := renvoObjectFloatConstantScaled(g.prog, ep, idx)
		size := renvoTypeSize(g.meta, typ)
		if !ok || size != 4 && size != 8 || offset+size > len(data) {
			return false
		}
		bits := renvoObjectFloat64BitsScaled(scaled)
		if size == 4 {
			bits = renvoObjectFloat32BitsScaled(scaled)
		}
		for at := 0; at < size; at++ {
...
func renvoObjectFloatConstantScaled(p *renvoProgram, ep *renvoExprParse, idx int) (int, bool) {
	renvoNonNil(p, ep)
	if idx < 0 || idx >= len(ep.exprs) {
		return 0, false
	}
	e := &ep.exprs[idx]
	if e.kind == renvoExprFloat {
		return renvoParseFloatTokenScaled(p, e.tok), true
	}
	if e.kind == renvoExprUnary && renvoTokCharIs(p, e.tok, '-') {
		value, ok := renvoObjectFloatConstantScaled(p, ep, e.left)
		return -value, ok
	}
	return 0, false
}

func renvoObjectFloat64BitsScaled(scaled int) int {
	if scaled == 0 {
		return 0
	}
	sign := 0
	...
	exponent := high - 2 + 1023
	return int(uint64(sign)<<63 | uint64(exponent)<<52 | significand&0xfffffffffffff)
}

func renvoObjectFloat32BitsScaled(scaled int) int {
	if scaled == 0 {
		return 0
	}
	sign := 0
	...
	exponent := high - 2 + 127
	return sign<<31 | exponent<<23 | int(significand&0x7fffff)
}
...
func renvoEmitIRQStackCall(g *renvoLinearGen, ep *renvoExprParse, e *renvoExpr, helper *renvoFuncInfo) bool {
	...
	if argCount == 2 {
		renvoAsmEmitText(&g.asm, "\x4c\x89\xee")
	}
...
func renvoEnsureCObjectFunctionPointerWrapper(g *renvoLinearGen, fnIndex int) (int, int, bool) {
	...
	if renvoPreparedBackend != 0 && ((sret || smallAggregateResult) || wordCount > registerWords || memoryAggregate) {
		return 0, 0, false
	}
...
func renvoEmitObjectExport(g *renvoLinearGen, fnIndex int) bool {
	...
	smallAggregateResult := renvoObjectExportUsesSmallAggregateResult(g.meta, fn)
	memoryAggregate := renvoObjectExportHasMemoryAggregate(g.meta, fn)
	if variadic && (wordCount < 1 || wordCount > 7 || renvoPreparedBackend != 0 || sret || smallAggregateResult) ||
		renvoPreparedBackend != 0 && ((sret || smallAggregateResult) || wordCount > 6 && !sret || wordCount > 5 && sret || memoryAggregate) {
		return false
	}
	symbolIndex := renvoAsmAddObjectFuncSymbol(
		&g.asm, g.prog.src, fn.exportNameStart, fn.exportNameEnd, wrapper, decl)
...
func renvoFunctionHasSingleDirectUse(g *renvoLinearGen, fnIndex int) bool {
	...
	for tok := 0; tok < renvoTokCount(p); tok++ {
		if !renvoTokIsKind(p, tok, renvoTokIdent) ||
			!renvoBytesEqualRange(p.src, renvoTokStart(p, tok), renvoTokEnd(p, tok), fn.nameStart, fn.nameEnd) {
			continue
		}
		if tok > 0 && renvoTokIsKind(p, tok-1, renvoTokFunc) {
			continue
		}
		if tok+1 >= renvoTokCount(p) || !renvoTokCharIs(p, tok+1, '(') {
...
func renvoEmitIntExpr(g *renvoLinearGen, ep *renvoExprParse, idx int) bool {
	...
	// Direct-source import lowering removes the unsafe qualifier but leaves the
	// Pointer call. It is an identity conversion at the backend boundary.
	if e.kind == renvoExprCall && e.argCount == 1 && renvoFuncInfoFromCall(g, ep, e.left) < 0 &&
		renvoExprIsIdentText(g.prog, ep, e.left, "Pointer") {
		return renvoEmitIntExpr(g, ep, renvo_runtime_UnsafeIntAt(ep.args, e.firstArg))
	}
	if e.kind == renvoExprCall && e.argCount == 1 {
		callee := &ep.exprs[e.left]
		if callee.kind == renvoExprSelector && renvoBytesEqualText(g.prog.src, callee.nameStart, callee.nameEnd, "Pointer") &&
			renvoExprIsIdentText(g.prog, ep, callee.left, "unsafe") {
			return renvoEmitIntExpr(g, ep, renvo_runtime_UnsafeIntAt(ep.args, e.firstArg))
		}
	}
...
func renvoEmitComplexValueRegs(g *renvoLinearGen, ep *renvoExprParse, idx int) bool {
	...
	if e.kind == renvoExprCall && renvoResolveType(g.meta, renvoInferParsedExprType(g, ep, idx)).kind == renvoTypeComplex {
		return renvoEmitUserCall(g, ep, idx)
	}
	if e.kind == renvoExprBinary && (renvoTokCharIs(g.prog, e.tok, '+') || renvoTokCharIs(g.prog, e.tok, '-') || renvoTokCharIs(g.prog, e.tok, '*') || renvoTokCharIs(g.prog, e.tok, '/')) {
		...
		if renvoTokCharIs(g.prog, e.tok, '*') || renvoTokCharIs(g.prog, e.tok, '/') {
			return renvoEmitComplexMultiplyDivide(g, leftReal, leftImag, rightReal, rightImag, e.tok)
		}
...
func renvoEmitComplexScaledProduct(g *renvoLinearGen, left int, right int) {
	renvoAsmLoadPrimaryTertiaryStack(&g.asm, right, left)
	renvoAsmMulPrimaryTertiary(g)
	renvoAsmSarPrimaryImm(&g.asm, 2)
}

func renvoEmitComplexProductPair(g *renvoLinearGen, firstLeft int, firstRight int, secondLeft int, secondRight int, subtract bool) {
	temp := renvoAddUnnamedLocal(g, renvoTypeFloat64)
	renvoEmitComplexScaledProduct(g, firstLeft, firstRight)
	renvoAsmStorePrimaryStack(&g.asm, temp)
	renvoEmitComplexScaledProduct(g, secondLeft, secondRight)
	if subtract {
		renvoAsmCopyPrimaryToTertiary(&g.asm)
...
func renvoEmitComplexMultiplyDivide(g *renvoLinearGen, leftReal int, leftImag int, rightReal int, rightImag int, tok int) bool {
	realPart := renvoAddUnnamedLocal(g, renvoTypeFloat64)
	imagPart := renvoAddUnnamedLocal(g, renvoTypeFloat64)
	multiply := renvoTokCharIs(g.prog, tok, '*')
	renvoEmitComplexProductPair(g, leftReal, rightReal, leftImag, rightImag, multiply)
	renvoAsmStorePrimaryStack(&g.asm, realPart)
	renvoEmitComplexProductPair(g, leftImag, rightReal, leftReal, rightImag, !multiply)
	renvoAsmStorePrimaryStack(&g.asm, imagPart)
	if !multiply {
		denominator := renvoAddUnnamedLocal(g, renvoTypeFloat64)
		renvoEmitComplexProductPair(g, rightReal, rightReal, rightImag, rightImag, false)
...
func renvoEmitComplexMemSecondaryRegs(g *renvoLinearGen, typ int) bool {
	if renvoResolveType(g.meta, typ).kind != renvoTypeComplex {
		return false
	}
	offset := renvoAddUnnamedLocal(g, typ)
	...
	if e.kind == renvoExprCall {
		...
		if e.left >= 0 && e.left < len(ep.exprs) && ep.exprs[e.left].kind == renvoExprIdent {
			...
			if renvoBytesPrefixText(p.src, calleeExpr.nameStart, calleeExpr.nameEnd, "__c_pointer_step_dec_") {
				return renvoEmitCPointerStep(g, ep, idx, e, true)
			}
		}
		if renvoExprIsIdentText(p, ep, e.left, "renvoNonNil") {
			return renvoEmitRuntimeTrustPointer(g, ep, e)
