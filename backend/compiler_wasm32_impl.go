...
// generator: 2
// target: algorithms/vm32
// unit: wasm32 cbcf94f2c9fb9c2869136b3d50ffacd7e34539dc3895f70db4c6a44adea94aea
package main
...
const renvoWasm32OpBinaryRegImm = 47
const renvoWasm32OpPushRegLoadStack = 48
const renvoWasm32OpLoadStackPushReg = 49
const renvoWasm32OpPushRegMovImm = 50
...
func renvoWasmAppendRecipe(out *renvoWasmBuffer, recipe string, p0 int, p1 int, p2 int, p3 int) {
	...
	for i := 0; i < len(recipe); i++ {
		...
		if command == 0 {
			...
			renvoWasm32LoadSized(out, value, 0)
		} else if command == 8 {
			renvoWasm32StoreSized(out, value, 0)
		} else if command == 9 {
...
func renvoWasm32EmitRegImm(a *renvoAsm, op int, reg int, imm int) {
	...
	if op == renvoWasm32OpMovRegImm && a.lastPrimaryLoad < 0 && marker/32 == end {
		pushed := marker % 32
		if pushed >= 0 && pushed < 8 && end >= 2 &&
			int(a.code[end-2]) == renvoWasm32OpPushReg && int(a.code[end-1]) == pushed {
			a.code = a.code[:end-2]
			a.code = append(a.code, byte(renvoWasm32OpPushRegMovImm), byte(pushed), byte(reg))
			a.code = renvoAppend32(a.code, imm)
			a.lastPrimaryLoad = 0
			return
		}
...
func rtgWasm32Wasm32PackageRenvoWasm32FusibleBinary(op int) bool {
	return op >= renvoWasm32OpAddRegReg && op <= renvoWasm32OpMulRegReg ||
		op >= renvoWasm32OpAndRegReg && op <= renvoWasm32OpShrRegReg ||
...
func renvoWasm32EmitRegReg(a *renvoAsm, op int, dst int, src int) {
	...
	if rtgWasm32Wasm32PackageRenvoWasm32FusibleBinary(op) &&
		...
		a.code = a.code[:end-6]
		a.code = append(a.code, byte(renvoWasm32OpBinaryRegImm), byte(dst))
		a.code = renvoAppend32(a.code, imm)
		a.code = append(a.code, byte(op))
		a.lastPrimaryLoad = 0
		return
	...
	if op == renvoWasm32OpPopReg {
		...
		if a.lastPrimaryLoad < 0 && marker/32 == end {
			...
			if pushed >= 0 && pushed < 8 && end >= 2 &&
				...
				a.lastPrimaryLoad = 0
				if pushed != reg {
					a.code = append(a.code, byte(renvoWasm32OpMovRegReg), byte(reg), byte(pushed))
				}
				return
			}
		}
		...
		if a.lastPrimaryLoad > 0 && positive/32 == end {
			...
			// LoadMem uses the same primary-load marker. Its displacement can
			// end in bytes that resemble a MovRegReg, so reject that longer
			// form before considering the three-byte move below.
	...
	if op == renvoWasm32OpPushReg {
		...
		if a.lastPrimaryLoad > 0 && marker/32 == end {
			if end >= 6 && int(a.code[end-6]) == renvoWasm32OpLoadStack {
				...
				offset := renvoGet32At(a.code, end-4)
				a.code = a.code[:end-6]
				a.code = append(a.code, byte(renvoWasm32OpLoadStackPushReg), byte(dst))
				a.code = renvoAppend32(a.code, offset)
				a.code = append(a.code, byte(reg))
				a.lastPrimaryLoad = 0
				return
			...
			if end >= 8 && int(a.code[end-8]) == renvoWasm32OpLoadMem {
				...
				size := int(a.code[end-1])
				a.code = a.code[:end-8]
				a.code = append(a.code, byte(renvoWasm32OpLoadMemPushReg), byte(loadReg), byte(base))
				a.code = renvoAppend32(a.code, disp)
				a.code = append(a.code, byte(size), byte(reg))
				a.lastPrimaryLoad = 0
				return
			}
		}
...
func renvoWasm32EmitStack(a *renvoAsm, op int, reg int, offset int) {
	...
	marker := -a.lastPrimaryLoad - 2
	if op == renvoWasm32OpLoadStack && a.lastPrimaryLoad < 0 && marker/32 == end {
		pushed := marker % 32
		if pushed >= 0 && pushed < 8 && end >= 2 &&
			int(a.code[end-2]) == renvoWasm32OpPushReg && int(a.code[end-1]) == pushed {
			a.code = a.code[:end-2]
			a.code = append(a.code, byte(renvoWasm32OpPushRegLoadStack), byte(pushed), byte(reg))
			a.code = renvoAppend32(a.code, offset)
			a.lastPrimaryLoad = 0
			return
		}
...
func renvoWasm32InstructionPcs(code []byte) []int {
	// The scratch slice grows when a very instruction-dense program needs it;
	// reserving the old 128K entries eagerly cost 256 KiB on ordinary builds.
...
func renvoWasmLocalGet(out *renvoWasmBuffer, local int) {
	if out.lastLocalOp == 0x21 && out.lastLocalIndex == local && out.lastLocalEnd == out.length {
		out.data[out.lastLocalStart] = 0x22
		out.lastLocalOp = 0x22
		return
	}
	start := out.length
	renvoWasmPut(out, 0x20)
	renvoWasmAppendU32(out, local)
	out.lastLocalOp = 0x20
	out.lastLocalIndex = local
	out.lastLocalStart = start
	out.lastLocalEnd = out.length
	return
}
func renvoWasmLocalSet(out *renvoWasmBuffer, local int) {
	if out.lastLocalOp == 0x20 && out.lastLocalIndex == local && out.lastLocalEnd == out.length {
		out.length = out.lastLocalStart
		out.lastLocalOp = 0
		return
	}
	start := out.length
...
func renvoWasmI32Load8S(out *renvoWasmBuffer) {
	renvoWasmPut(out, 0x2c)
	renvoWasmAppendU32(out, 0)
	renvoWasmAppendU32(out, 0)
	return
}
...
func renvoWasmI32Load16U(out *renvoWasmBuffer) {
	renvoWasmPut(out, 0x2f)
	renvoWasmAppendU32(out, 1)
	renvoWasmAppendU32(out, 0)
	return
}
...
func renvoWasm32StackAddr(out *renvoWasmBuffer, offset int) {
	renvoWasmLocalGet(out, rtgWasm32Wasm32PackageRenvoWasm32LocalFrameBase)
	delta := out.frameSize - offset
	if delta > 0 {
		renvoWasmAppendI32Const(out, delta)
		renvoWasmPut(out, 0x6a)
	}
	return
}
func rtgWasm32Wasm32PackageRenvoWasm32AppendPushReg(out *renvoWasmBuffer, reg int) {
	renvoWasmLocalGet(out, renvoWasm32LocalSp)
	renvoWasm32RegGet(out, reg)
...
func rtgWasm32Wasm32PackageRenvoWasm32AppendPopReg(out *renvoWasmBuffer, reg int) {
	renvoWasmLocalGet(out, renvoWasm32LocalSp)
	renvoWasmAppendI32Const(out, 4)
	renvoWasmPut(out, 0x6b)
	renvoWasmLocalTee(out, renvoWasm32LocalSp)
	renvoWasmI32Load(out, 2, 0)
	renvoWasm32RegSet(out, reg)
}
func rtgWasm32Wasm32PackageRenvoWasm32AppendLoadStackReg(out *renvoWasmBuffer, reg int, offset int) {
	renvoWasmLocalGet(out, rtgWasm32Wasm32PackageRenvoWasm32LocalFrameBase)
	renvoWasmI32Load(out, 2, out.frameSize-offset)
	renvoWasm32RegSet(out, reg)
}
func renvoWasm32StackI64(out *renvoWasmBuffer, offset int) {
	renvoWasmLocalGet(out, rtgWasm32Wasm32PackageRenvoWasm32LocalFrameBase)
	renvoWasmPut(out, 0x29)
	renvoWasmAppendU32(out, 3)
	renvoWasmAppendU32(out, out.frameSize-offset)
	return
}
...
func renvoWasm32LoadSized(out *renvoWasmBuffer, size int, off int) {
	op := 0x28
	align := 2
	if size == 129 {
		op = 0x2c
	...
	renvoWasmPut(out, byte(op))
	renvoWasmAppendU32(out, align)
	renvoWasmAppendU32(out, off)
	return
}
func renvoWasm32StoreSized(out *renvoWasmBuffer, size int, off int) {
	op := 0x36
	align := 2
	if size == 1 {
		op = 0x3a
...
func renvoWasm32Patch(a *renvoAsm, dataBase int, bssBase int) {
	...
	for i := 0; i+2 < len(a.absRelocs); i += 3 {
		...
		if kind == 1 {
			...
			target = bssBase + renvoAlignValue(a.bssSize, alignment)
		}
		renvoPut32At(a.code, at, target)
	}
}
...
func renvoWasm32AppendInstr(out *renvoWasmBuffer, code []byte, pc int, nextIndex int, targetIndex int, loopDepth int) {
	...
	if op == renvoWasm32OpMemoryCopy {
		...
		// memory.copy is byte-accurate and handles either overlap direction.
	...
	if op == renvoWasm32OpWideBinary {
		...
		if wasmOp >= 0x92 && wasmOp <= 0x98 {
			// Float32 values occupy the low word of a wide frame slot. Convert
			// between that raw representation and WebAssembly's native f32 type.
			...
			// Float64 values already fill a wide slot; only reinterpret their bits.
			recipe = "\xff\x06\x00\xff\x0b\x01\xbf\xff\x0b\x02\xbf\xff\x0e\x03\xbd\xff\x0d\x00"
		} else if wasmOp == 0xb2 || wasmOp == 0xb3 {
			// i32 -> f32
			recipe = "\xff\x06\x00\xff\x0b\x01\xa7\xff\x0e\x03\xbc\xad\xff\x0d\x00"
		} else if wasmOp == 0xb4 || wasmOp == 0xb5 {
			// i64 -> f32
			recipe = "\xff\x06\x00\xff\x0b\x01\xff\x0e\x03\xbc\xad\xff\x0d\x00"
		} else if wasmOp == 0xb7 || wasmOp == 0xb8 {
			// i32 -> f64
			recipe = "\xff\x06\x00\xff\x0b\x01\xa7\xff\x0e\x03\xbd\xff\x0d\x00"
		} else if wasmOp == 0xb9 || wasmOp == 0xba {
			// i64 -> f64
			recipe = "\xff\x06\x00\xff\x0b\x01\xff\x0e\x03\xbd\xff\x0d\x00"
		} else if wasmOp == 0xb6 {
			// f64 -> f32
			recipe = "\xff\x06\x00\xff\x0b\x01\xbf\xff\x0e\x03\xbc\xad\xff\x0d\x00"
		} else if wasmOp == 0xbb {
			// f32 -> f64
			recipe = "\xff\x06\x00\xff\x0b\x01\xa7\xbe\xff\x0e\x03\xbd\xff\x0d\x00"
		} else if wasmOp == 0xa8 || wasmOp == 0xa9 {
			// f32 -> i32
			recipe = "\xff\x06\x00\xff\x0b\x01\xa7\xbe\xff\x0e\x03\xad\xff\x0d\x00"
		} else if wasmOp == 0xaa || wasmOp == 0xab {
			// f64 -> i32
			recipe = "\xff\x06\x00\xff\x0b\x01\xbf\xff\x0e\x03\xad\xff\x0d\x00"
		} else if wasmOp == 0xae || wasmOp == 0xaf {
			// f32 -> i64
			recipe = "\xff\x06\x00\xff\x0b\x01\xa7\xbe\xff\x0e\x03\xff\x0d\x00"
		} else if wasmOp == 0xb0 || wasmOp == 0xb1 {
			// f64 -> i64
	...
	if op == renvoWasm32OpWideCompare {
		wasmOp := int(renvo_runtime_UnsafeByteAt(code, pc+9))
		recipe := "\xff\x0b\x00\xff\x0b\x01\xff\x0e\x02\xff\x05\x03"
		if wasmOp >= 0x5b && wasmOp <= 0x60 {
			recipe = "\xff\x0b\x00\xa7\xbe\xff\x0b\x01\xa7\xbe\xff\x0e\x02\xff\x05\x03"
		} else if wasmOp >= 0x61 && wasmOp <= 0x66 {
			recipe = "\xff\x0b\x00\xbf\xff\x0b\x01\xbf\xff\x0e\x02\xff\x05\x03"
		}
		renvoWasmAppendRecipe(out, recipe, renvoWasm32GetS32(code, pc+1), renvoWasm32GetS32(code, pc+5), wasmOp, renvoWasm32RegRax)
		return
	}
	if op == renvoWasm32OpBinaryRegImm {
		dst := int(renvo_runtime_UnsafeByteAt(code, pc+1))
		imm := renvoWasm32GetS32(code, pc+2)
		binaryOp := int(renvo_runtime_UnsafeByteAt(code, pc+6))
		renvoWasm32RegGet(out, dst)
		if binaryOp == renvoWasm32OpAndNotRegReg {
			renvoWasmAppendI32Const(out, ^imm)
			renvoWasmPut(out, 0x71)
		} else {
			renvoWasmAppendI32Const(out, imm)
			renvoWasm32AppendBinaryOp(out, binaryOp)
		}
		renvoWasm32RegSet(out, dst)
		if loopDepth >= 0 {
			renvoWasm32SetPc(out, nextIndex)
			renvoWasmBr(out, loopDepth)
		}
	...
	if op >= renvoWasm32OpPushRegLoadStack && op <= renvoWasm32OpMovRegRegPop {
		if op == renvoWasm32OpPushRegLoadStack {
		...
		} else if op == renvoWasm32OpLoadStackPushReg {
			...
			rtgWasm32Wasm32PackageRenvoWasm32AppendPushReg(out, int(renvo_runtime_UnsafeByteAt(code, pc+6)))
		} else if op == renvoWasm32OpPushRegMovImm {
			rtgWasm32Wasm32PackageRenvoWasm32AppendPushReg(out, int(renvo_runtime_UnsafeByteAt(code, pc+1)))
			renvoWasmAppendI32Const(out, renvoWasm32GetS32(code, pc+3))
			renvoWasm32RegSet(out, int(renvo_runtime_UnsafeByteAt(code, pc+2)))
		} else if op == renvoWasm32OpLoadMemPushReg {
			renvoWasm32MemAddr(out, int(renvo_runtime_UnsafeByteAt(code, pc+2)), renvoWasm32GetS32(code, pc+3))
			renvoWasm32LoadSized(out, int(renvo_runtime_UnsafeByteAt(code, pc+7)), 0)
			renvoWasm32RegSet(out, int(renvo_runtime_UnsafeByteAt(code, pc+1)))
			rtgWasm32Wasm32PackageRenvoWasm32AppendPushReg(out, int(renvo_runtime_UnsafeByteAt(code, pc+8)))
	...
	if op == renvoWasm32OpLoadStack {
		renvoWasmLocalGet(out, rtgWasm32Wasm32PackageRenvoWasm32LocalFrameBase)
		renvoWasmI32Load(out, 2, out.frameSize-renvoWasm32GetS32(code, pc+2))
		renvoWasm32RegSet(out, int(renvo_runtime_UnsafeByteAt(code, pc+1)))
		if loopDepth >= 0 {
			renvoWasm32SetPc(out, nextIndex)
			renvoWasmBr(out, loopDepth)
	...
	if op == renvoWasm32OpStoreStack {
		renvoWasmLocalGet(out, rtgWasm32Wasm32PackageRenvoWasm32LocalFrameBase)
		renvoWasm32RegGet(out, int(renvo_runtime_UnsafeByteAt(code, pc+1)))
		renvoWasmI32Store(out, 2, out.frameSize-renvoWasm32GetS32(code, pc+2))
		if loopDepth >= 0 {
			renvoWasm32SetPc(out, nextIndex)
			renvoWasmBr(out, loopDepth)
		}
	...
	if recipeAt != 0 {
		...
		renvoWasm32MemAddr(out, int(renvo_runtime_UnsafeByteAt(code, pc+2)), renvoWasm32GetS32(code, pc+3))
		renvoWasm32RegGet(out, int(renvo_runtime_UnsafeByteAt(code, pc+1)))
		renvoWasm32StoreSized(out, int(renvo_runtime_UnsafeByteAt(code, pc+7)), 0)
		...
		renvoWasm32LoadSized(out, int(renvo_runtime_UnsafeByteAt(code, pc+9)), 0)
		renvoWasm32RegSet(out, int(renvo_runtime_UnsafeByteAt(code, pc+1)))
	} else if op == renvoWasm32OpStoreIndex {
		renvoWasm32IndexAddr(out, int(renvo_runtime_UnsafeByteAt(code, pc+2)), int(renvo_runtime_UnsafeByteAt(code, pc+3)), int(renvo_runtime_UnsafeByteAt(code, pc+4)), renvoWasm32GetS32(code, pc+5))
		renvoWasm32RegGet(out, int(renvo_runtime_UnsafeByteAt(code, pc+1)))
		renvoWasm32StoreSized(out, int(renvo_runtime_UnsafeByteAt(code, pc+9)), 0)
...
func renvoWasm32AppendInstrDirect(out *renvoWasmBuffer, code []byte, pc int, nextIndex int, targetIndex int, loopDepth int, frameSize int, routinePcs []int, symbolPcs []int, callArgBase int) {
	op := int(renvo_runtime_UnsafeByteAt(code, pc))
	if op == renvoWasm32OpExit {
		renvoWasmLocalGet(out, renvoWasm32LocalRax)
		renvoWasmAppendCall(out, renvoWasm32ImportProcExit)
	...
	if op == renvoWasm32OpCall {
		...
		if argB > 6 && callArgBase > 0 {
			rtgWasm32Wasm32PackageRenvoWasm32AppendLocalCallArgs(out, callArgBase, argB-6)
		}
...
func renvoWasm32RoutineEnds(routinePcs []int, symbolPcs []int, code []byte, instrPcs []int) []int {
	...
	// Only the result escapes this analysis. Reuse the per-instruction
	// visitation map and worklist storage while emitting the module.
	...
	for i := 0; i < len(routinePcs); i++ {
		...
		// A helper's first return may precede a reachable error path.
		// Follow its control flow so allocation failures and fault exits
		// remain in the emitted function after the successful return.
		pending = pending[:0]
...
func renvoWasm32RoutineFrameSize(code []byte, pcs []int) int {
	...
	for i := 0; i < len(pcs); i++ {
		...
		} else if op == renvoWasm32OpPushRegLoadStack {
			offset := renvoWasm32GetS32(code, pc+3)
			if offset > frameSize {
				frameSize = offset
			}
		} else if op == renvoWasm32OpLoadStackPushReg || op == renvoWasm32OpLoadStackPop {
			offset := renvoWasm32GetS32(code, pc+2)
			if offset > frameSize {
				frameSize = offset
			}
...
func renvoWasm32AppendDirectRoutineBody(body *renvoWasmBuffer, code []byte, pcs []int, routinePcs []int, symbolPcs []int, frameSize int, directLocals []int32) {
	blockStarts := renvoWasm32BuildBlockStartsLocal(code, pcs)
	localStack, localStackCount := rtgWasm32Wasm32PackageRenvoWasm32LocalStackMap(code, pcs, blockStarts)
	renvoWasmAppendU32(body, 1)
	renvoWasmAppendU32(body, 8+localStackCount+len(directLocals))
	renvoWasmPut(body, 0x7f)
	renvoWasmLocalGet(body, renvoWasm32LocalFp)
	if frameSize > 0 {
		renvoWasmAppendI32Const(body, frameSize)
		renvoWasmPut(body, 0x6b)
	}
	renvoWasmLocalSet(body, rtgWasm32Wasm32PackageRenvoWasm32LocalFrameBase)
	directLocalBase := 17 + localStackCount
	if len(blockStarts) == 1 {
		for i := 0; i < len(pcs); i++ {
			...
			if op == renvoWasm32OpLoadStack || op == renvoWasm32OpStoreStack {
				directLocal = rtgWasm32Wasm32PackageRenvoWasm32DirectLocal(directLocals, renvoWasm32GetS32(code, pc+2), directLocalBase)
			}
			if directLocal >= 0 {
				rtgWasm32Wasm32PackageRenvoWasm32AppendDirectLocalOp(body, code, pc, directLocal)
				continue
			}
			mayFuse := op == renvoWasm32OpPushReg || op == renvoWasm32OpPushImm || op == renvoWasm32OpStoreStack
			if mayFuse && i+1 < len(pcs) && renvoWasm32CanFusePair(code, pc, pcs[i+1]) {
				renvoWasm32AppendFusedPair(body, code, pc, pcs[i+1])
				i++
				continue
			}
			if localStack[i] != 0 && (op == renvoWasm32OpPushReg || op == renvoWasm32OpPushImm || op == renvoWasm32OpPopReg) {
				rtgWasm32Wasm32PackageRenvoWasm32AppendLocalStackOp(body, code, pc, localStack[i])
				continue
			}
			renvoWasm32AppendInstrDirect(body, code, pc, 0, 0, -1, frameSize, routinePcs, symbolPcs, localStack[i])
		}
	...
	loopHeaders, loopTails, structured := rtgWasm32Wasm32PackageRenvoWasm32RoutineStructure(code, pcs, blockStarts, instrBlockIndex)
	if structured {
		labels := make([]int, 0, len(blockStarts)+len(loopHeaders))
		for target := len(blockStarts); target >= 1; target-- {
			if rtgWasm32Wasm32PackageRenvoWasm32BlockRegion(target, loopHeaders, loopTails) >= 0 {
				continue
			}
			renvoWasmAppend2(body, 0x02, 0x40)
			labels = append(labels, target)
		}
		for block := 0; block < len(blockStarts); block++ {
			if block > 0 {
				renvoWasmPut(body, 0x0b)
				labels = labels[:len(labels)-1]
			}
			loop := rtgWasm32Wasm32PackageRenvoWasm32LoopAt(block, loopHeaders)
			if loop >= 0 {
				renvoWasmAppend2(body, 0x03, 0x40)
				labels = append(labels, -block-1)
				for target := loopTails[loop]; target > block; target-- {
					if rtgWasm32Wasm32PackageRenvoWasm32BlockRegion(target, loopHeaders, loopTails) != loop {
						continue
					}
					renvoWasmAppend2(body, 0x02, 0x40)
					labels = append(labels, target)
			...
			start := blockStarts[block]
			end := renvoWasm32BlockEnd(blockStarts, block, len(pcs))
			for i := start; i < end; i++ {
				pc := pcs[i]
				op := int(renvo_runtime_UnsafeByteAt(code, pc))
				directLocal := -1
				...
				if i+1 == end && (renvoWasm32OpHasTarget(op) || op == renvoWasm32OpJCond) {
					targetPC := renvoGet32At(code, pc+1)
					if op == renvoWasm32OpJCond {
						targetPC = renvoGet32At(code, pc+2)
					}
					target := renvoWasm32PcLowerBound(pcs, targetPC)
					targetBlock := renvoWasm32BlockForInstrFast(instrBlockIndex, target)
					wanted := targetBlock
					if targetBlock <= block {
						wanted = -targetBlock - 1
					}
					depth := -1
					for label := len(labels) - 1; label >= 0; label-- {
						if labels[label] == wanted {
							depth = len(labels) - 1 - label
							break
						}
					}
					if depth >= 0 {
						rtgWasm32Wasm32PackageRenvoWasm32AppendStructuredBranch(body, code, pc, depth)
					}
					continue
				}
				renvoWasm32AppendInstrDirect(body, code, pc, 0, 0, -1, frameSize, routinePcs, symbolPcs, localStack[i])
			}
			for i := len(loopTails) - 1; i >= 0; i-- {
				if loopTails[i] == block {
					renvoWasmPut(body, 0x0b)
					labels = labels[:len(labels)-1]
				}
			}
	...
	for blockIndex := len(blockStarts) - 1; blockIndex >= 0; blockIndex-- {
		...
		for i < end {
			...
			if directLocal >= 0 {
				rtgWasm32Wasm32PackageRenvoWasm32AppendDirectLocalOp(body, code, pc, directLocal)
				if i+1 >= end {
					nextBlock := renvoWasm32BlockForInstrFast(instrBlockIndex, i+1)
			...
			if i+1 < end {
				...
				renvoWasm32AppendInstrDirect(body, code, pc, nextBlock, targetBlock, blockIndex, frameSize, routinePcs, symbolPcs, localStack[i])
			}
			i++
		}
	}
...
func renvoWasm32TypeSectionFull() []byte {
	return []byte("")
}
...
func renvoWasm32AppendDirectRoutine(out *renvoWasmBuffer, code []byte, instrPcs []int, routinePcs []int, symbolPcs []int, directLocals []int32) {
	...
	renvoWasm32AppendDirectRoutineBody(out, code, instrPcs, routinePcs, symbolPcs, frameSize, directLocals)
...
func rtgWasm32Wasm32PackageRenvoWasm32AppendDirectLocalOp(out *renvoWasmBuffer, code []byte, pc int, local int) {
	reg := int(renvo_runtime_UnsafeByteAt(code, pc+1))
	if int(renvo_runtime_UnsafeByteAt(code, pc)) == renvoWasm32OpLoadStack {
		renvoWasmLocalGet(out, local)
		renvoWasm32RegSet(out, reg)
		return
	}
	renvoWasm32RegGet(out, reg)
	renvoWasmLocalSet(out, local)
}
func rtgWasm32Wasm32PackageRenvoWasm32AppendLocalCallArgs(out *renvoWasmBuffer, base int, count int) {
	for i := 0; i < count; i++ {
		renvoWasmLocalGet(out, renvoWasm32LocalSp)
		renvoWasmLocalGet(out, 16+base+i)
		renvoWasmI32Store(out, 2, i*4)
	}
	renvoWasmLocalGet(out, renvoWasm32LocalSp)
	renvoWasmAppendI32Const(out, count*4)
	renvoWasmPut(out, 0x6a)
	renvoWasmLocalSet(out, renvoWasm32LocalSp)
}
func rtgWasm32Wasm32PackageRenvoWasm32AppendLocalStackOp(out *renvoWasmBuffer, code []byte, pc int, slot int) {
	op := int(renvo_runtime_UnsafeByteAt(code, pc))
	if slot < 0 {
		if op == renvoWasm32OpPushReg {
			renvoWasm32RegGet(out, int(renvo_runtime_UnsafeByteAt(code, pc+1)))
		...
		if op == renvoWasm32OpPushImm {
			renvoWasmAppendI32Const(out, renvoWasm32GetS32(code, pc+1))
			return
		}
		renvoWasm32RegSet(out, int(renvo_runtime_UnsafeByteAt(code, pc+1)))
		return
...
func rtgWasm32Wasm32PackageRenvoWasm32AppendStructuredBranch(out *renvoWasmBuffer, code []byte, pc int, depth int) {
	op := int(renvo_runtime_UnsafeByteAt(code, pc))
	if op == renvoWasm32OpJmp {
		renvoWasmBr(out, depth)
		return
	}
	if op == renvoWasm32OpJz {
		renvoWasmLocalGet(out, renvoWasm32LocalFlag)
	...
	renvoWasmBrIf(out, depth)
}
func rtgWasm32Wasm32PackageRenvoWasm32BlockRegion(block int, headers []int, tails []int) int {
	region := -1
	for i := 0; i < len(headers); i++ {
		if block > headers[i] && block <= tails[i] {
			region = i
		}
	}
	return region
}
func rtgWasm32Wasm32PackageRenvoWasm32DirectLocal(offsets []int32, offset int, base int) int {
	for i := 0; i < len(offsets); i++ {
		if int(offsets[i]) == offset {
			return base + i
		}
	}
	return -1
}
func rtgWasm32Wasm32PackageRenvoWasm32DirectLocalsForRoutine(a *renvoAsm, routinePC int) []int32 {
	slots := a.wasmLocalSlots
	for i := 0; i+1 < len(slots); {
		pc := int(slots[i])
		count := int(slots[i+1])
		if pc == routinePC {
			return slots[i+2 : i+2+count]
		}
		i += count + 2
	}
	return nil
}
func rtgWasm32Wasm32PackageRenvoWasm32LocalStackMap(code []byte, pcs []int, starts []int) ([]int, int) {
	locals := make([]int, len(pcs))
	maxCallArgs := 0
	stack := make([]int, 0, 16)
	for block := 0; block < len(starts); block++ {
		end := renvoWasm32BlockEnd(starts, block, len(pcs))
		stack = stack[:0]
		for i := starts[block]; i < end; i++ {
			op := int(renvo_runtime_UnsafeByteAt(code, pcs[i]))
			if op >= renvoWasm32OpPushRegLoadStack && op <= renvoWasm32OpMovRegRegPop {
				// Fused instructions retain their physical VM stack effects. Do
				// not match local-stack substitutions across one of them.
				stack = stack[:0]
				continue
			}
			if op == renvoWasm32OpCall {
				extra := renvoGet32At(code, pcs[i]+5) - 6
				if extra < 0 {
					extra = 0
				}
				if extra > len(stack) {
					...
					// Earlier pushes can belong to an enclosing call whose later
					// argument is this call. Keep those live values in distinct
					// local slots until the enclosing call consumes them.
					if extra > 0 {
						locals[i] = base + 1
					}
					for depth := 0; depth < extra; depth++ {
						locals[stack[base+depth]] = base + depth + 1
					}
					if extra > 0 && base+extra > maxCallArgs {
						maxCallArgs = base + extra
					}
					stack = stack[:len(stack)-extra]
				}
				continue
			}
			if op == renvoWasm32OpPushReg || op == renvoWasm32OpPushImm {
				stack = append(stack, i)
				continue
			}
			if op != renvoWasm32OpPopReg || len(stack) == 0 {
				continue
			}
			depth := len(stack)
			push := stack[depth-1]
	...
	return locals, maxCallArgs
}
func rtgWasm32Wasm32PackageRenvoWasm32LoopAt(header int, headers []int) int {
	for i := 0; i < len(headers); i++ {
		if headers[i] == header {
			return i
		}
...
func rtgWasm32Wasm32PackageRenvoWasm32RoutineStructure(code []byte, pcs []int, blockStarts []int, blockIndex []int) ([]int, []int, bool) {
	headers := make([]int, 0, 8)
	tails := make([]int, 0, 8)
	for block := 0; block < len(blockStarts); block++ {
		end := renvoWasm32BlockEnd(blockStarts, block, len(pcs))
		if end <= blockStarts[block] {
			continue
		}
		pc := pcs[end-1]
		op := int(renvo_runtime_UnsafeByteAt(code, pc))
		...
		target := renvoWasm32PcLowerBound(pcs, targetPC)
		targetBlock := renvoWasm32BlockForInstrFast(blockIndex, target)
		if targetBlock <= block {
			found := -1
			for i := 0; i < len(headers); i++ {
				if headers[i] == targetBlock {
					found = i
					break
				}
			}
			if found < 0 {
				headers = append(headers, targetBlock)
				tails = append(tails, block)
			} else if block > tails[found] {
				tails[found] = block
			}
		}
	}
	...
	for i := 1; i < len(headers); i++ {
		for outer := i - 1; outer >= 0; outer-- {
			if headers[i] > tails[outer] {
				break
			}
			if tails[i] > tails[outer] {
				tails[outer] = tails[i]
			}
		}
	}
	for block := 0; block < len(blockStarts); block++ {
		...
		target := renvoWasm32BlockForInstrFast(blockIndex, renvoWasm32PcLowerBound(pcs, targetPC))
		if target <= block {
			valid := false
			for i := 0; i < len(headers); i++ {
				if target == headers[i] && block <= tails[i] {
					valid = true
					break
				}
			}
		...
		for i := 0; i < len(headers); i++ {
			if target > headers[i] && target <= tails[i] && (block < headers[i] || block > tails[i]) {
				return nil, nil, false
			}
		}
	}
