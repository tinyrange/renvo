...
// generator: 2
// target: compiler-integration/x86_64
// unit: x86_64_compiler 417a64b9062fdcb95fb9539c3878f34b90ae37a6230c4fd37c09046c444bcc15
package main
...
func renvoAmd64PushStackBytes(a *renvoAsm, offset int, size int) {
	// Reserve the same argument words as the descending PUSH sequence. Locals
	// live in the persistent frame above the old RSP, so the new argument area
	// is disjoint and a forward copy preserves their ascending byte order.
	// Call argument registers are populated after all arguments are stacked.
	renvoAsmEmitText(a, "\x48\x81\xec") // SUB RSP, size
	renvoAsmEmit32(a, size)
	renvoAsmAddressCallWord1Stack(a, offset) // LEA RSI, [RBP-offset]
	renvoAsmEmitText(a, "\x48\x89\xe7\xb9")  // MOV RDI, RSP; MOV ECX, size
	renvoAsmEmit32(a, size)
	renvoAsmEmitText(a, "\xfc\xf3\xa4") // CLD; REP MOVSB
}
