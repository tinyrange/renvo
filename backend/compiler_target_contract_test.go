...
func TestCompilerSourceManifestCoversBackendImplementationFiles(t *testing.T) {
	...
	// Statically prepared projections are selected by dedicated preparation
	// build tags. They must not enter the ordinary bootstrap source manifest,
	// whose stage compilers consume sources without host Go's file-selection
	// pass.
	...
	for _, path := range implementationFiles {
		...
		if bytes.HasPrefix(source, []byte("//go:build renvo_prepared\n")) ||
			bytes.HasPrefix(source, []byte("//go:build renvo_jvm_prepared\n")) ||
			bytes.HasPrefix(source, []byte("//go:build renvo_prepared || renvo_jvm_prepared\n")) {
			continue
		}
