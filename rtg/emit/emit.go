...
func Source(u unit.Unit) []byte {
	out := make([]byte, 0, sourceCapacity(u))
	...
	out = append(out, '\n')
	if u.Entry {
		out = appendString(out, "// rtg:entrypoint\n")
	}
	...
	for i := 0; i < len(u.Decls); i++ {
		...
		out = append(out, '\n')
		body := trimTrailingNewlines(decl.Body)
		out = appendString(out, body)
...
func sourceCapacity(u unit.Unit) int {
	capacity := 256
	capacity += len(u.ImportPath) + len(u.Package)
	imports := u.Imports
	for i := 0; i < len(imports); i++ {
		imp := imports[i]
		capacity += len(imp) + 16
	...
	for i := 0; i < len(exports); i++ {
		sym := exports[i]
		capacity += len(sym.Name) + len(sym.UnitName) + 24
	}
	references := u.References
	for i := 0; i < len(references); i++ {
		sym := references[i]
		capacity += len(sym.ImportPath) + len(sym.Name) + len(sym.UnitName) + 24
	}
	decls := u.Decls
	for i := 0; i < len(decls); i++ {
		decl := decls[i]
		capacity += len(decl.Kind) + len(decl.Name) + len(decl.UnitName) + len(decl.Path) + len(decl.Body) + 40
	}
	if capacity < 1024 {
		return 1024
	}
...
func symbolAfterByName(a unit.Symbol, b unit.Symbol) bool {
	if a.Name == b.Name {
		return stringGreater(a.UnitName, b.UnitName)
	}
