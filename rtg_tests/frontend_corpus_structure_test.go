...
func TestFrontendCorpusStructuralCoverage(t *testing.T) {
	root := repoRoot(t)
	data, err := os.ReadFile(filepath.Join(root, "rtg_tests", "corpus_manifest.json"))
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, group := range manifest.Groups {
		...
		t.Run(group.Tier+"/"+group.Group, func(t *testing.T) {
			dir := filepath.Join(root, "rtg_tests", group.Tier, group.Group)
			...
			if cases != group.Cases || len(shapes) != group.StructuralShapes {
				t.Fatalf("manifest drift: cases=%d shapes=%d, want cases=%d shapes=%d; run go run ./rtg_tests/generate_tests.go", cases, len(shapes), group.Cases, group.StructuralShapes)
			}
