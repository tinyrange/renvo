...
func TestCH32V003SeparatesObjectAndBoardContracts(t *testing.T) {
	...
	if composition.Board.Runtime.Result.Transport != ResultTransportDebuggerMemory || composition.Board.Runtime.Result.Symbol != "rtgres" {
...
func TestCompositionDerivesC89ProfileFromObjectContract(t *testing.T) {
	...
	for _, want := range [][]byte{
		[]byte("#define RTG_C_HOSTED 0"),
		[]byte("RTG_C89_ASSERT(pointer_width, sizeof(void *) * CHAR_BIT == 32)"),
		[]byte("#define RTG_C_TARGET_ABI \"ilp32e\""),
		[]byte("#define RTG_C_RUNTIME_RESULT 1"),
	} {
