...
//export c_mix
func c_mix(left int, right int, scale int) int { return 0 }
`))
	...
	header := cgoExportHeader([]syntax.File{file}, []goExportDefinition{definition}, c11.DataModelLP64)
	if !bytes.Contains(header, []byte("extern long long c_mix(long long, long long, long long);")) {
		t.Fatalf("LP64 export header = %q", header)
	}
	header = cgoExportHeader([]syntax.File{file}, []goExportDefinition{definition}, c11.DataModelILP32)
	if !bytes.Contains(header, []byte("extern int c_mix(int, int, int);")) {
		t.Fatalf("ILP32 export header = %q", header)
	}
}
