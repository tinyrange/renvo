...
func normalizeSourceFiles(files []SourceFile) ([]SourceFile, int) {
	...
	for i := 0; i < len(files); i++ {
		out = append(out, SourceFile{Path: CleanPath(files[i].Path), Src: files[i].Src, CPrelude: files[i].CPrelude, CObject: files[i].CObject, ArenaStart: files[i].ArenaStart, ArenaEnd: files[i].ArenaEnd})
	}
	sortSourceFiles(out)
	for i := 1; i < len(out); i++ {
