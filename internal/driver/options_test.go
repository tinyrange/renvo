...
func TestCommandHelpRequested(t *testing.T) {
	...
	for _, want := range []string{"Usage: renvo", "-o <file>", "-system <file.rtg>", "-mode=<mode>", "kernel-module", "file.go...", "Exactly the named files", "windows/amd64", "windows/arm64", "darwin/arm64", "wasi/wasm32", "vm/bytecode"} {
