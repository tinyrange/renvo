...
const objectDefaultArenaSize = 65536

// A bundled frontend can self-host within the checked 128 MiB system profile.
// Leave another 128 MiB for a long-lived host to retain its application state
// while invoking the embedded compiler.
...
func backendArenaSize(target string, tags []string, requested int, mode string) int {
	if requested != 0 {
		return requested
	}
	if mode == ModeObject {
		return objectDefaultArenaSize
	}
...
.renvo-title{height:30px;flex:0 0 30px;display:flex;align-items:center;padding-left:11px;background:#303744;color:#fff;cursor:move;user-select:none}.renvo-title span{flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.renvo-close{width:42px;height:30px;border:0;background:transparent;color:#fff;font:18px sans-serif}.renvo-close:hover{background:#c42b3a}.renvo-client{position:relative;display:flex;flex:1 1 auto;min-height:0}.renvo-canvas{display:block;width:100%;height:100%;min-height:0;outline:none}.renvo-accessibility{position:absolute;inset:0;overflow:hidden;pointer-events:none}.renvo-accessibility>*{position:absolute;box-sizing:border-box;margin:0;padding:0;opacity:0;pointer-events:none}.renvo-terminal{box-sizing:border-box;flex:1;margin:0;padding:14px;overflow:auto;background:#12151a;color:#e8e8e8;white-space:pre-wrap}
</style></head><body><div id="desktop"></div><script>
"use strict";const wasm64="`

const browserHTMLSuffix = `";
const desktop=document.getElementById("desktop"),decoder=new TextDecoder(),encoder=new TextEncoder(),EXIT={};let topZ=10,rootContext=null,nextSemanticContextID=1;
...
function fdRead(ctx,fd,at,count,read){const parts=iovecs(ctx,at,count),view=contextView(ctx);if(fd===0){if(ctx.input.length===0){view.setUint32(read,0,true);return 0}const source=ctx.input[0];let used=0;for(const part of parts){const size=Math.min(part.length,source.length-used);part.set(source.subarray(used,used+size));used+=size;if(used===source.length)break}if(used===source.length)ctx.input.shift();else ctx.input[0]=source.subarray(used);view.setUint32(read,used,true);return 0}const file=ctx.fds.get(fd);if(!file||file.kind!=="file")return 8;view.setUint32(read,readFile(ctx,file,parts),true);return 0}
...
function importsFor(ctx){const args=["app"],env=["PWD="+ctx.cwd];return{wasi_snapshot_preview1:{fd_write:(...a)=>fdWrite(ctx,...a),fd_read:(...a)=>fdRead(ctx,...a),fd_pread:(...a)=>fdPread(ctx,...a),fd_pwrite:(...a)=>fdPwrite(ctx,...a),path_open:(...a)=>pathOpen(ctx,...a),fd_close:fd=>fdClose(ctx,fd),fd_fdstat_get:(fd,at)=>fdStat(ctx,fd,at),fd_readdir:(...a)=>fdReaddir(ctx,...a),args_sizes_get:(c,s)=>{const v=contextView(ctx);v.setUint32(c,args.length,true);v.setUint32(s,stringSizes(args),true);return 0},args_get:(p,d)=>writeStrings(ctx,args,p,d),environ_sizes_get:(c,s)=>{const v=contextView(ctx);v.setUint32(c,env.length,true);v.setUint32(s,stringSizes(env),true);return 0},environ_get:(p,d)=>writeStrings(ctx,env,p,d),proc_exit:()=>{throw EXIT}}}}
function graphicsData(ctx,part){if(ctx.pending){const pending=ctx.pending;ctx.pending=null;if(part.length!==pending.length)return;if(pending.kind==="window")createWindow(ctx,pending.id,pending.width,pending.height,decoder.decode(part));else if(pending.kind==="frame")presentWindow(ctx,pending.id,pending.width,pending.height,part);else if(pending.kind==="title")setWindowTitle(ctx,pending.id,decoder.decode(part));else if(pending.kind==="accessibility")applyAccessibility(ctx,pending.id,part);return}if(part.length<8)return;const magic=String.fromCharCode(part[0],part[1],part[2],part[3]),d=new DataView(part.buffer,part.byteOffset,part.byteLength),id=d.getUint32(4,true);if(magic==="RNVX"){closeWindow(ctx,id);return}if(magic==="RNVW"&&part.length===20){const pending={kind:"window",id,width:d.getUint32(8,true),height:d.getUint32(12,true),length:d.getUint32(16,true)};if(pending.length===0)createWindow(ctx,id,pending.width,pending.height,"");else ctx.pending=pending}else if(magic==="RNVF"&&part.length===20)ctx.pending={kind:"frame",id,width:d.getUint32(8,true),height:d.getUint32(12,true),length:d.getUint32(16,true)};else if(magic==="RNVT"&&part.length===12)ctx.pending={kind:"title",id,length:d.getUint32(8,true)};else if(magic==="RNVA"&&part.length===12)ctx.pending={kind:"accessibility",id,length:d.getUint32(8,true)};else if(magic==="RNVM"&&part.length===16)setWindowTimer(ctx,id,d.getUint32(8,true),d.getUint32(12,true));else if(magic==="RNVC"&&part.length===12)cancelWindowTimer(ctx,id,d.getUint32(8,true))}
function makeShell(ctx,title,width,height,terminal=false){const shell=document.createElement("div"),root=ctx.root&&ctx.windows.size===0,windowTitle=title||"Renvo Application";shell.className="renvo-window "+(root?"root":"floating");shell.setAttribute("role",root?"application":"dialog");shell.setAttribute("aria-label",windowTitle);if(!root){shell.setAttribute("aria-modal","false");shell.style.width=width+"px";shell.style.height=(height+30)+"px";const bar=document.createElement("div");bar.className="renvo-title";const caption=document.createElement("span");caption.textContent=windowTitle;const close=document.createElement("button");close.className="renvo-close";close.textContent="×";close.setAttribute("aria-label","Close "+windowTitle);bar.append(caption,close);shell.append(bar);shell.caption=caption;shell.closeButton=close;close.onclick=()=>{if(shell.view)enqueue(ctx,1);shell.remove()};dragWindow(shell,bar)}desktop.append(shell);focusWindow(shell);if(terminal){const pre=document.createElement("pre");pre.className="renvo-terminal";shell.append(pre);return{shell,terminal:pre,root}}const client=document.createElement("div");client.className="renvo-client";const canvas=document.createElement("canvas");canvas.className="renvo-canvas";canvas.tabIndex=0;const accessibility=document.createElement("div");accessibility.className="renvo-accessibility";client.append(canvas,accessibility);shell.append(client);return{shell,canvas,accessibility,root}}
...
function createWindow(ctx,id,width,height,title){const made=makeShell(ctx,title,width,height),view={id,ctx,...made,width,height,gl:null,texture:null,semantic:new Map(),semanticRevision:0,semanticElementID:0,patchingSemantics:false};made.shell.view=view;ctx.windows.set(id,view);wireCanvas(view);made.shell.addEventListener("pointerdown",e=>{if(e.target!==made.canvas)enqueue(ctx,4)});if(view.root)requestAnimationFrame(()=>{resizeView(view);view.canvas.focus()});else{new ResizeObserver(()=>resizeView(view)).observe(view.canvas);requestAnimationFrame(()=>view.canvas.focus())}}
...
function setWindowTitle(ctx,id,title){const view=ctx.windows.get(id);if(!view)return;view.shell.setAttribute("aria-label",title);if(view.shell.caption)view.shell.caption.textContent=title;if(view.shell.closeButton)view.shell.closeButton.setAttribute("aria-label","Close "+title)}
...
function presentWindow(ctx,id,width,height,pixels){const view=ctx.windows.get(id);if(!view)return;if(!view.gl)initGL(view);const gl=view.gl,scale=Math.max(1,window.devicePixelRatio||1),pixelWidth=Math.round(width*scale),pixelHeight=Math.round(height*scale);view.canvas.width=pixelWidth;view.canvas.height=pixelHeight;view.width=width;view.height=height;gl.viewport(0,0,pixelWidth,pixelHeight);gl.bindTexture(gl.TEXTURE_2D,view.texture);gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA,width,height,0,gl.RGBA,gl.UNSIGNED_BYTE,pixels);gl.drawArrays(gl.TRIANGLES,0,6)}
const semanticRoles=["group","","button","textbox","checkbox","radio","img","menubar","menu","menuitem","separator","tree","treeitem","list","listitem","status"];
function semanticElement(view,node){let tag="div";if(node.role===1||node.role===15)tag="span";else if(node.role===2)tag="button";else if(node.role===3)tag=(node.state&128)!==0?"textarea":"input";else if(node.role===4||node.role===5)tag="input";const element=document.createElement(tag);element.id="renvo-a11y-"+view.ctx.semanticContextID+"-"+view.id+"-"+(++view.semanticElementID);if(node.role===4)element.type="checkbox";else if(node.role===5)element.type="radio";element.dataset.renvoId=node.id;element.addEventListener("click",()=>{const item=element.renvoNode;if(item&&(item.actions&1)!==0)enqueue(view.ctx,15,0,0,0,0,1,0,0,0,item.id)});element.addEventListener("focus",()=>{const item=element.renvoNode;if(item&&(item.actions&2)!==0)enqueue(view.ctx,15,0,0,0,0,2,0,0,0,item.id)});element.addEventListener("input",()=>{const item=element.renvoNode;if(item&&(item.actions&4)!==0)enqueue(view.ctx,15,0,0,0,0,3,0,0,0,item.id+String.fromCharCode(0)+element.value)});element.addEventListener("select",()=>{const item=element.renvoNode;if(!view.patchingSemantics&&item&&(item.actions&8)!==0){const start=utf16ToUTF8(element.value,element.selectionStart),end=utf16ToUTF8(element.value,element.selectionEnd);enqueue(view.ctx,15,0,0,0,0,4,0,0,0,item.id+String.fromCharCode(0)+start+":"+end)}});view.accessibility.append(element);view.semantic.set(node.id,element);return element}
function removeSemantic(view,id){const element=view.semantic.get(id);if(element){element.remove();view.semantic.delete(id)}for(const [childID,child] of view.semantic)if(child.renvoNode&&child.renvoNode.parent===id)removeSemantic(view,childID)}
function utf8ToUTF16(text,offset){let bytes=0,units=0;for(const value of text){const size=encoder.encode(value).length;if(bytes+size>offset)break;bytes+=size;units+=value.length}return units}
function utf16ToUTF8(text,offset){return encoder.encode(text.slice(0,offset)).length}
function patchSemanticSelection(view,id,start,end){const element=view.semantic.get(id);if(!element||!element.setSelectionRange)return;element.renvoNode.selectionStart=start;element.renvoNode.selectionEnd=end;element.setSelectionRange(utf8ToUTF16(element.value,start),utf8ToUTF16(element.value,end))}
function patchSemanticElement(view,node){let element=view.semantic.get(node.id),expected=node.role===1||node.role===15?"SPAN":node.role===2?"BUTTON":node.role===3?((node.state&128)!==0?"TEXTAREA":"INPUT"):node.role===4||node.role===5?"INPUT":"DIV";if(element&&element.tagName!==expected){element.remove();view.semantic.delete(node.id);element=null}if(!element)element=semanticElement(view,node);element.renvoNode=node;element.dataset.renvoParent=node.parent;const role=semanticRoles[node.role]||"";if(role)element.setAttribute("role",role);else element.removeAttribute("role");if(node.name)element.setAttribute("aria-label",node.name);else element.removeAttribute("aria-label");if(node.description)element.setAttribute("aria-description",node.description);else element.removeAttribute("aria-description");element.hidden=(node.state&1)!==0;element.tabIndex=(node.actions&2)!==0?0:-1;element.style.left=node.x+"px";element.style.top=node.y+"px";element.style.width=Math.max(0,node.width)+"px";element.style.height=Math.max(0,node.height)+"px";const disabled=(node.state&2)!==0;element.setAttribute("aria-disabled",disabled?"true":"false");if("disabled" in element)element.disabled=disabled;if((node.state&8)!==0){const checked=(node.state&16)!==0;element.setAttribute("aria-checked",checked?"true":"false");if("checked" in element)element.checked=checked}else element.removeAttribute("aria-checked");if((node.state&32)!==0)element.setAttribute("aria-selected",(node.state&64)!==0?"true":"false");else element.removeAttribute("aria-selected");if((node.actions&4)!==0&&document.activeElement!==element&&element.value!==node.value)element.value=node.value;if(node.selectionStart>=0&&node.selectionEnd>=node.selectionStart&&element.setSelectionRange)element.setSelectionRange(utf8ToUTF16(node.value,node.selectionStart),utf8ToUTF16(node.value,node.selectionEnd));if(node.role===3)element.setAttribute("aria-multiline",(node.state&128)!==0?"true":"false");if(node.role===1||node.role===15)element.textContent=node.name||node.value;element.dataset.focused=(node.state&4)!==0?"true":"false"}
function syncSemanticOwnership(view){for(const element of view.semantic.values()){element.removeAttribute("aria-owns");element.removeAttribute("aria-controls");element.removeAttribute("aria-activedescendant");if(element.renvoNode&&element.renvoNode.role===3){element.setAttribute("aria-expanded","false");element.removeAttribute("aria-autocomplete")}}for(const [parentID,parent] of view.semantic){const children=[];for(const child of view.semantic.values())if(child.renvoNode&&child.renvoNode.parent===parentID)children.push(child);if(children.length===0)continue;if(parent.renvoNode&&parent.renvoNode.role===3){const list=children.find(child=>child.renvoNode.role===13);if(list){parent.setAttribute("aria-controls",list.id);parent.setAttribute("aria-expanded","true");parent.setAttribute("aria-autocomplete","list");const selected=Array.from(view.semantic.values()).find(child=>child.renvoNode&&child.renvoNode.parent===list.renvoNode.id&&(child.renvoNode.state&64)!==0);if(selected)parent.setAttribute("aria-activedescendant",selected.id)}}else parent.setAttribute("aria-owns",children.map(child=>child.id).join(" "))}}
function applyAccessibility(ctx,id,data){const view=ctx.windows.get(id);if(!view||data.length<20)return;const d=new DataView(data.buffer,data.byteOffset,data.byteLength);let at=0;const u32=()=>{if(at+4>data.length)throw Error("invalid accessibility patch");const value=d.getUint32(at,true);at+=4;return value},i32=()=>{if(at+4>data.length)throw Error("invalid accessibility patch");const value=d.getInt32(at,true);at+=4;return value},str=()=>{const length=u32();if(at+length>data.length)throw Error("invalid accessibility string");const value=decoder.decode(data.subarray(at,at+length));at+=length;return value};try{if(u32()!==1)return;const revision=u32(),flags=u32();if(revision<=view.semanticRevision)return;view.semanticRevision=revision;view.patchingSemantics=true;if((flags&1)!==0){view.accessibility.replaceChildren();view.semantic.clear()}const removed=u32();for(let i=0;i<removed;i++)removeSemantic(view,str());const replaced=u32();for(let i=0;i<replaced;i++){const parentID=str(),children=[];for(const [childID,child] of view.semantic)if(child.renvoNode&&child.renvoNode.parent===parentID)children.push(childID);for(const childID of children)removeSemantic(view,childID)}const selections=u32();for(let i=0;i<selections;i++)patchSemanticSelection(view,str(),i32(),i32());const count=u32();for(let i=0;i<count;i++){const node={id:str(),parent:str(),role:u32(),name:str(),description:str(),value:str(),x:i32(),y:i32(),width:i32(),height:i32(),actions:u32(),state:u32(),selectionStart:i32(),selectionEnd:i32()};patchSemanticElement(view,node)}syncSemanticOwnership(view);view.patchingSemantics=false}catch(e){view.patchingSemantics=false;showError(ctx,e)}}
...
async function startContext(binary,root,cwd="."){const ctx={root,cwd,input:[],pending:null,windows:new Map(),timers:new Map(),fds:new Map(),nextFd:8,memory:null,instance:null,terminal:null,semanticContextID:nextSemanticContextID++};if(root)rootContext=ctx;try{const result=await WebAssembly.instantiate(binary,importsFor(ctx));ctx.instance=result.instance;ctx.memory=result.instance.exports.memory;try{ctx.instance.exports._start()}catch(e){if(e!==EXIT)throw e}return ctx}catch(e){showError(ctx,e);return ctx}}
