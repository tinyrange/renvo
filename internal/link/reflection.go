...
func lowerReflectionCore(program *unit.Program, names *coreReflectionNames, transient bool) bool {
	...
	file, _ := syntax.ParseLinkedFile(source)
	if !file.Ok {
		return false
	}
	...
	for i := 0; i < len(file.Decls); i++ {
		...
		for j := 0; j < len(fields); j++ {
			...
			descriptor += "{Name:" + string(appendCoreQuotedString(nil, fieldName)) + ",Tag:" + string(appendCoreQuotedString(nil, check.StructFieldTag(&file, field))) + "},"
