package apk

var sha256Initial = [8]uint32{
	0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
	0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19,
}

var sha256Constants = [64]uint32{
	0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
	0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
	0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3,
	0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
	0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc,
	0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
	0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7,
	0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
	0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13,
	0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
	0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3,
	0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
	0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5,
	0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
	0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208,
	0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
}

func sha256Digest(data []byte) []byte {
	state := sha256Initial
	blocks := (len(data) + 9 + 63) / 64
	message := make([]byte, blocks*64)
	for i := 0; i < len(data); i++ {
		message[i] = data[i]
	}
	message[len(data)] = 0x80
	bits := uint64(len(data)) * 8
	for i := 0; i < 8; i++ {
		message[len(message)-1-i] = byte(bits >> (i * 8))
	}
	var words [64]uint32
	for block := 0; block < len(message); block += 64 {
		for i := 0; i < 16; i++ {
			at := block + i*4
			words[i] = uint32(message[at])<<24 |
				uint32(message[at+1])<<16 |
				uint32(message[at+2])<<8 |
				uint32(message[at+3])
		}
		for i := 16; i < 64; i++ {
			x := words[i-15]
			y := words[i-2]
			s0 := rotateRight32(x, 7) ^ rotateRight32(x, 18) ^ x>>3
			s1 := rotateRight32(y, 17) ^ rotateRight32(y, 19) ^ y>>10
			words[i] = words[i-16] + s0 + words[i-7] + s1
		}
		a, b, c, d := state[0], state[1], state[2], state[3]
		e, f, g, h := state[4], state[5], state[6], state[7]
		for i := 0; i < 64; i++ {
			s1 := rotateRight32(e, 6) ^ rotateRight32(e, 11) ^ rotateRight32(e, 25)
			choose := (e & f) ^ ((e ^ 0xffffffff) & g)
			t1 := h + s1 + choose + sha256Constants[i] + words[i]
			s0 := rotateRight32(a, 2) ^ rotateRight32(a, 13) ^ rotateRight32(a, 22)
			majority := (a & b) ^ (a & c) ^ (b & c)
			t2 := s0 + majority
			h, g, f, e = g, f, e, d+t1
			d, c, b, a = c, b, a, t1+t2
		}
		state[0] += a
		state[1] += b
		state[2] += c
		state[3] += d
		state[4] += e
		state[5] += f
		state[6] += g
		state[7] += h
	}
	out := make([]byte, 0, 32)
	for i := 0; i < 8; i++ {
		out = append(out, byte(state[i]>>24), byte(state[i]>>16),
			byte(state[i]>>8), byte(state[i]))
	}
	return out
}

func rotateRight32(value uint32, amount uint) uint32 {
	return value>>amount | value<<(32-amount)
}
