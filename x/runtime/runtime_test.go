...
func (h *recordingHandler) ChanSend(uintptr, unsafe.Pointer) int     { return int(h.sendStatus) }
func (h *recordingHandler) ChanReceive(uintptr, unsafe.Pointer) bool { return true }
...
func TestRuntimeWrappersDelegateToHandler(t *testing.T) {
	...
	if !renvo_runtime_ChanReceive(17, nil) || renvo_runtime_ChanLen(17) != 3 || renvo_runtime_ChanCap(17) != 5 {
...
func TestRuntimeWrappersReportUsefulPanics(t *testing.T) {
	tests := []struct {
		...
		{name: "closed send", call: func() { activeHandler = &recordingHandler{sendStatus: StatusClosed}; renvo_runtime_ChanSend(1, nil) }},
