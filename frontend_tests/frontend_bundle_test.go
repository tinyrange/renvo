...
func TestBundledFrontendStandaloneAllTargets(t *testing.T) {
	...
	targets := []struct {
		...
		{name: "vm/bytecode", artifact: "renvo-vm-bytecode.rnvb", prefix: []byte{'R', 'N', 'V', 'B'}},
	}
	...
	for _, want := range []string{"Usage: renvo", "file.go...", "Exactly the named files", "Targets:", "windows/amd64", "darwin/arm64", "wasi/wasm32", "vm/bytecode"} {
